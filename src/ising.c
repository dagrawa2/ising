...
void help()
{
    ...
    fprintf(stderr,"  ising [ -d 4 ] [ -L 20 ] [ -J 1 ] [ -T 4.0 ] [ [-m | --nmeas ] 20 ] [ [ -c | --nmcs ] 10000000 ] \n");
    fprintf(stderr," Help:\n");
    ...
    fprintf(stderr,"  ising -d 4 -L 20 -J 1 -T 4.0 --nmeas 20 --nmcs 10000000 --ieq 5000\n");
    return;
}
...
*/
int parseinput(int argc, char **argv,int *d, int *L, int *J, double *T, \
               ...
               unsigned long int *seed,int *ieq)
{
    ...
    while (1)
    {
        ...
        int option_index = 0;
        c = getopt_long (argc, argv, "hpD:d:l:L:J:j:T:t:m:c:s:y:", long_options, &option_index);
...
void BondsAround(IntVector spins, IntVector cluster, ListIntVector veins, ListIntVector bonds, int around, double expo, int J)
{
    int i;
...
void ClusteringNeigh(IntVector spins, IntVector cluster, ListIntVector veins, ListIntVector bonds, double expo, int spin, int J)
{
    int i;
    IntVector vei,bond;
    ...
    BondsAround(spins,cluster,veins,bonds,spin,expo,J); //Search for occupied bonds of this spin
    
    for (i=0;i<vei[-1];++i)
    {
        if (bond[i] == 1 && cluster[vei[i]] == 0) //For each bond occupied and unvisited neighbors...
        {
            ClusteringNeigh(spins,cluster,veins,bonds,expo,vei[i],J);
        }
    }
}
...
void WolffMetropolis(IntVector spins, ListIntVector veins, ListIntVector bonds, double expo, int steps, int J)
{
    int initial;
    int spinscluster;
    int stp;
    ...
    for (stp=0;stp<steps;++stp)
    {
        for (totalspins=0;totalspins<N;) //We flip, at least, N spins
        {
            ...
            ClusteringNeigh(spins,cluster,veins,bonds,expo,initial,J); //Create the cluster
...
int main(int argc, char **argv)
{
    ...
    parseinput(argc,argv,&d,&L,&J,&T,&nmeas,&nmcs,&seed,&ieq); //Function that read the inputs
    printf("Running with parameters: d: %d\t L: %d\t J: %d\t T: %f\t nmeas: %d \t ieq: %d \t nmcs: %ld\n",d,L,J,T,nmeas,ieq,nmcs); //Print the inputs
    makedir("output");
    FILE *json_fitx=Fopen("output/args.json", "w"); // record cmd args in JSON
    fprintf(json_fitx, "{\n  \"d\": %d,\n  \"L\": %d,\n  \"J\": %d,\n  \"T\": %f,\n  \"nmeas\": %d,\n  \"nmcs\": %ld,\n  \"s\": %d,\n  \"ieq\": %d\n}", d, L, J, T, nmeas, nmcs, seed, ieq);
    Fclose(json_fitx);
    ...
    {
        WolffMetropolis(spins,veins,bonds,expo,nmeas,J); //Metropolis for "nmcs" MCS taking measures each "nmeas"
